...
:root{
  ...
  /* 嵌入态补偿变量（2026-09-22 语义更新）：
     --embed-bottom 恒 0 —— 手机端壳 .sys-area 的 bottom 已让出「TabBar + 安全区」，
     iframe 底边与 TabBar 顶边贴合，模块若再让一次就会在列表底部撑出空白死带。
     --embed-top 由壳注入「主内容容器实际顶边」，供模块按真实占位算高。 */
  --embed-bottom:0px;
  --embed-top:0px;
}
...
/* ---- 嵌入态高度（2026-09-16 首修 / 2026-09-22 复核重构）----
   内联 max-height:calc(100vh - 160px) 在壳内嵌时按模块自身视口算，与 iframe 实际高度
   不符；且模块顶栏在嵌入态被壳 display:none 隐藏，公式里为它预留的 150px 会整段变成
   列表下方的空白死带（手机端实测 113px，刘海屏因安全区重复补偿更大）。
   现分两层兜底：
   ① 兜底：壳注入实测顶部占位 --embed-top（默认 96px），按 iframe 视口精确算高；
   ② 精确：壳的 fitEmbedMain() 另行写内联 max-height(important)，随 render/resize 校正。 */
.main-scroll-container{
  max-height:calc(100vh - 160px - var(--embed-bottom,0px))!important;
  padding-bottom:calc(16px + var(--embed-bottom,0px))!important;
}
html[data-embed="1"] .main-scroll-container{
  max-height:calc(100dvh - var(--embed-top,96px) - var(--embed-bottom,0px))!important;
  padding-bottom:16px!important;
}
@media (max-width:760px){
  /* 独立打开（无壳）时顶部占位 = 顶栏 + 导航 ≈ 158px，旧值 150 会让容器底边
     溢出轨 8px（2026-09-22 实测）；对齐 >760px 档的 160，留 2px 余量。 */
  .main-scroll-container{max-height:calc(100dvh - 160px - var(--embed-bottom,0px))!important;}
}
"""
